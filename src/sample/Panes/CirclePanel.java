...
public class CirclePanel extends InteractivePane {
    ...
    private final static int MAX_MOVEMENT = 50;
    private final static int MASS = 500_000; //in grams
...
//    private final static int SENSITIVE_DISTANCE = 300;
    ...
    public void init() {
        double radius = 2 * DIST_FROM_CENTER * Math.PI / (CIRCLE_PER_LAYER * 2);
        for (int layer = 0; layer < LAYERS; layer++) {
            for (int i = 0; i <= CIRCLE_PER_LAYER; i++) {
                double deg = i / (double) (CIRCLE_PER_LAYER + 1) * 2 * Math.PI;
    ...
    private void moveCircle(double mx, double my, Circle circle) {
        ...
        double force = MASS / Math.pow(distance, 2);    //Calculation of gravity
